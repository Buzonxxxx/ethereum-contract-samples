...
contract IFCToken is StandardToken {

    // metadata
    string public constant name = "Infinite Chain";
    ...
    address public ifcFundDeposit;      // deposit address for XPlay Ltd. use and IFC User Fund
    ...
    uint256 public tokenExchangeRate = 23000;   // 23000 IFC tokens per 1 ETH
    uint256 public constant tokenCreationCap =  10 * (10**9) * 10**decimals;
    uint256 public tokenCrowdsaleCap =  4 * (10**8) * 10**decimals;

    // events
    event CreateIFC(address indexed _to, uint256 _value);

    // constructor
    function IFCToken(
        address _ethFundDeposit,
        address _ifcFundDeposit,
        ...
        public
    {
        ...
        ifcFundDeposit = _ifcFundDeposit;
        tokenExchangeRate = _tokenExchangeRate;
        fundingStartBlock = _fundingStartBlock;
        fundingEndBlock = _fundingEndBlock;
        totalSupply = tokenCreationCap;
        balances[ifcFundDeposit] = tokenCreationCap;    // deposit all IFC to XPlay Ltd.
        CreateIFC(ifcFundDeposit, tokenCreationCap);    // logs deposit of XPlay Ltd. fund
    }
    ...
    function () payable public {
        ...
        balances[msg.sender] += tokens;     // add amount of IFC to sender
        balances[ifcFundDeposit] = safeSub(balances[ifcFundDeposit], tokens); // subtracts amount from XPlay's balance
        CreateIFC(msg.sender, tokens);      // logs token creation

    }
    /// @dev Accepts ether and creates new IFC tokens.
